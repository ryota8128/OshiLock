...
export default function HomeScreen() {
  ...
  const urgentCards = MOCK_CARDS.filter((c) => {
    if (!c.schedule.startDate) return false;
    const diff = new Date(c.schedule.startDate).getTime() - now;
    return diff > 0 && diff <= 48 * 60 * 60 * 1000;
  });
  const todayCards = MOCK_CARDS.filter((c) => c.schedule.startDate !== null).slice(0, 2);
